...
namespace ADL.Combat.Player
{
    public class PlayerMonoBehaviour : MonoBehaviour, IHitboxOwner
    {
        ...
        public void Update()
        {
            ...
            Vector2 position = Helpers.RoundToPixel(transform.position, Configuration.PIXELS_PER_UNIT);
            HeadSprite.transform.position = position;
            BodySprite.transform.position = position;
            WeaponSprite.transform.position = position;

            int sortingOrder = Helpers.SortingOrder(position);
        ...
        public IEnumerator LayPlayerTrail()
        {
            ...
            while (Manager.Sprinting)
            {
                ...
                int sortingOrder = Helpers.SortingOrder(newHeadSprite.transform.position);
